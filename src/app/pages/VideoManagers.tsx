...
const VideoManager: React.FC = () => {
  ...
  const handleFileUpload = async (event: React.ChangeEvent<HTMLInputElement>, type: 'intro' | 'trailer' | 'other') => {
    ...
    try {
      ...
      // Upload the video
      const uploadedVideo = await uploadVideo(file, type);
    ...
    try {
      ...
      setVideos(prev => prev.map(video => 
        video.id === editingVideo.id 
          ? { ...video, name: editForm.name, description: editForm.description }
          : video
      ));
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
        <div className="bg-gray-50 dark:bg-gray-800 p-6 rounded-lg">
          ...
          <p className="text-sm text-gray-600 dark:text-gray-300 mb-4">
            This video will be shown in the hero section and should be a short loop.
          </p>
          <label className="block w-full">
            <div className="relative">
              ...
              <button 
                ...
                onClick={() => (document.querySelectorAll('input[type="file"]')[0] as HTMLInputElement).click()}
                disabled={uploading}
              >
                <Upload size={18} />
        ...
        <div className="bg-gray-50 dark:bg-gray-800 p-6 rounded-lg">
          ...
          <p className="text-sm text-gray-600 dark:text-gray-300 mb-4">
            This video will be shown when the user clicks "Watch Trailer" in fullscreen mode.
          </p>
          <label className="block w-full">
            <div className="relative">
              <input 
                ...
                <tr key={video.id} className="hover:bg-gray-50 dark:hover:bg-gray-700">
                  <td className="px-6 py-4 whitespace-nowrap">
                    {editingVideo?.id === video.id ? (
                      ...
                      <div className="flex items-center space-x-3">
                        ...
                        <div>
                          <div className="font-medium text-gray-900 dark:text-white">{video.name}</div>
                          {video.description && (
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full 
                      ${video.type === 'intro' ? 'bg-green-100 text-green-800' : 
                        video.type === 'trailer' ? 'bg-blue-100 text-blue-800' : 
